...
export async function POST(req: NextRequest) {
  try {
    ...
    const url = `${getBackendUrl()}/api/auth/recover`
    ...
    console.error("[Proxy auth/recover]", e.message)
    return NextResponse.json({ error: e.message || "Error en recuperación" }, { status: 500 })
  }
}