    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8">
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-2 pt-8">
          ...
          <p className="text-slate-400">4 pasos para tener tu propio sistema de envío masivo</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {STEPS.map((s, i) => (
            <div key={s.id} className="flex-1 flex items-center gap-2">
              <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${step >= s.id ? "bg-emerald-500 text-white" : "bg-slate-800 text-slate-500"}`}>
                {step > s.id ? "✓" : s.id}
              </div>
              <div className="hidden sm:block">
                <p className={`text-sm font-medium ${step >= s.id ? "text-white" : "text-slate-500"}`}>{s.title}</p>
            ...
            <div className="space-y-3 text-sm text-slate-300">
              ...
              <ol className="list-decimal list-inside space-y-2 ml-2">
                ...
                <li>En <strong>Variables</strong>, agregá:
                  <ul className="ml-4 mt-1 space-y-1 text-slate-400 font-mono text-xs">
        ...
        {step === 4 && !activated && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            ...
            <p className="text-sm text-slate-400">Pegá acá la key que recibiste por email al comprar.</p>
            ...
            <textarea
              ...
              rows={4}
              className="w-full bg-slate-950 border border-slate-700 rounded-xl p-4 font-mono text-xs text-white placeholder:text-slate-600 focus:outline-none focus:border-emerald-500"
            />
            ...
            <button 
              ...
              className={`w-full font-bold py-3 rounded-xl transition-colors ${licenseKey.trim() ? "bg-emerald-600 hover:bg-emerald-500 text-white" : "bg-slate-800 text-slate-500 cursor-not-allowed"}`}
            >
