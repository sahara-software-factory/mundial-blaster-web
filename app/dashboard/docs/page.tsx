...
const DOC_TREE: DocSection[] = [
  ...
  },
  {
    ...
    icon: BarChart3,
    articles: [
      { id: "dashboard-read", title: "Leer el dashboard", slug: "dashboard-read", difficulty: "beginner", plan: "free", readTime: "4 min" },
    ...
    case "welcome":
      return (
        <>
          ...
          <DocImage src="/docs/welcome-hero.png" alt="WabiSend Dashboard" caption="Dashboard principal de WabiSend v3.12" />
    ...
    case "install":
      return (
        <>
          ...
          <DocStep number={1} title="Clonar el repositorio">
            Descargá el código fuente desde tu repositorio privado o el template que te proporcionamos.
          </DocStep>
          <DocCode code="git clone https://github.com/tu-org/wabisend.git\ncd wabisend" lang="bash" />
          <DocStep number={2} title="Instalar dependencias">
            Usamos <code className="text-cyan-400 bg-slate-800 px-1 rounded">pnpm</code> para gestión de paquetes. Asegurate de tenerlo instalado.
          </DocStep>
          <DocCode code="pnpm install" lang="bash" />
          <DocStep number={3} title="Configurar variables de entorno">
            Copiá el archivo de ejemplo y completá tus credenciales.
          </DocStep>
          <DocCode code="cp .env.example .env\n# Editar .env con tu editor favorito" lang="bash" />
          <DocCallout type="tip">
            Las variables mínimas son: <code className="text-cyan-400">DATABASE_URL</code>, <code className="text-cyan-400">JWT_SECRET</code>, <code className="text-cyan-400">NEXT_PUBLIC_API_URL</code> y <code className="text-cyan-400">BACKEND_URL</code>.
          </DocCallout>
          <DocStep number={4} title="Ejecutar migraciones de Prisma">
            Esto crea las tablas en tu base de datos PostgreSQL.
          </DocStep>
          <DocCode code="npx prisma migrate dev\nnpx prisma generate" lang="bash" />
          <DocStep number={5} title="Iniciar servicios">
            Levantá el backend y el frontend en terminales separadas, o usá un proceso manager como PM2.
          </DocStep>
          <DocCode code="# Terminal 1 - Backend\npnpm --filter backend start\n\n# Terminal 2 - Frontend\npnpm --filter frontend dev" lang="bash" />
          <DocChecklist items={[
            { label: "Repositorio clonado", checked: false },
            { label: "Dependencias instaladas", checked: false },
            { label: "Variables de entorno configuradas", checked: false },
            { label: "Base de datos migrada", checked: false },
            { label: "Servicios corriendo", checked: false },
          ]} />
    ...
    case "setup-wizard":
      return (
        <>
          ...
          <DocImage src="/docs/setup-wizard.png" alt="Setup Wizard" caption="Pantalla inicial del wizard de configuración" />
    ...
    case "first-line":
      return (
        <>
          ...
          <DocImage src="/docs/lines-page.png" alt="Página de líneas" />
    ...
    case "create-basic":
      return (
        <>
          ...
          <DocImage src="/docs/campaign-create.png" alt="Crear campaña" />
          ...
          <DocCallout type="danger">
            Antes de enviar a miles de contactos, usá siempre el <strong>Modo Simulacro Lite</strong> para verificar que la línea esté activa y el mensaje se vea correctamente.
          </DocCallout>
    ...
    case "sim-lite":
      return (
        <>
          ...
          <p className="text-slate-300 leading-relaxed mb-4">
            El <strong>Simulacro Lite</strong> envía un único mensaje de prueba al primer contacto de la lista. Sirve para validar formato, imagen y conectividad sin riesgo.
          </p>
          <DocImage src="/docs/sim-lite.png" alt="Simulacro Lite" />
    ...
    case "dashboard-read":
      return (
        <>
          ...
          <p className="text-slate-300 leading-relaxed mb-4">
            El <strong>Dashboard</strong> es el centro de comando de WabiSend. Desde acá controlás campañas activas, líneas conectadas, estadísticas en tiempo real y alertas del sistema.
          </p>
          <DocImage src="/docs/dashboard-overview.png" alt="Dashboard WabiSend" />
    ...
    case "control-room":
      return (
        <>
          ...
          <DocImage src="/docs/control-room.png" alt="Sala de Control" />
    ...
    case "replies":
      return (
        <>
          ...
          <DocCallout type="tip">
            Los números que respondieron se cruzan automáticamente con tu tabla de <strong>Contactos</strong>. Si el número está agendado, se muestra el nombre al lado: <span className="text-emerald-400">5492604621921 (Yasin)</span>.
          </DocCallout>
    ...
    case "caleb":
      return (
        <>
          ...
          <DocImage src="/docs/caleb-ai.png" alt="Asistente Caleb" />
      ...
      return (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            {[
              { name: "Starter", price: "$250", color: "border-slate-600", badge: "Free", features: ["1 línea WhatsApp", "Campañas básicas", "Reportes simples", "Blacklist manual", "Import CSV", "Tags"] },
              { name: "Pro", price: "$500", color: "border-cyan-500/40", badge: "Pro", popular: true, features: ["Todo Starter +", "3 líneas", "Programación", "Spintax", "Proxy rotate", "Modo humano", "Export CSV", "Métricas avanzadas", "Respuestas tracking"] },
              { name: "Business", price: "$750", color: "border-purple-500/40", badge: "Business", features: ["Todo Pro +", "Round Robin", "Multi-usuario", "Asistente IA", "Campañas recurrentes", "Whitelist", "Simulacro Full", "Affiliate system"] },
            ].map((plan, i) => (
              <div key={i} className={`relative p-5 rounded-xl bg-slate-800/30 border ${plan.color} ${plan.popular ? "ring-1 ring-cyan-500/20" : ""}`}>
                {plan.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-cyan-500 text-white text-[10px] font-bold uppercase tracking-wider">
                    Más popular
                  </div>
                )}
      ...
      return (
        <>
          ...
          <DocStep number={1} title="Obtener código">
            Andá a <strong>Ganá plata</strong> en el sidebar. Tu código affiliate se muestra automáticamente. Ej: <code className="text-cyan-400 bg-slate-800 px-1 rounded">WS-YASIN-2026</code>.
          </DocStep>
